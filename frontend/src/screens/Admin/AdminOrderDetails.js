...
import { useDeliverOrderMutation } from "../../redux/slices/adminSlice";
...
function AdminOrderDetails() {
  ...
  const [deliverOrder, { isLoading: deliverLoading, isError: deliverError }] =
  ...
  const markAsDelivered = async () => {
    try {
       const markAsDeliveredd = await deliverOrder(id).unwrap();
      refetch();
           ...
           ${order?.shippingAddress?.postalCode}  ${order?.shippingAddress?.country}`}{" "}
                ...
                <div className="payment border-bottom pb-5 pt-4">
                  ...
                  {order?.orderItems.map((orders) => (
                    <div
                      className="orderbox p-4 text-center border"
                      key={orders.name}
                    >
